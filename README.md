...
Если в параметре `count` указано больше, чем есть всего, должны вернуться все доступные кафе. В сервисе будет только один город `moscow`, в котором будет всего 4 кафе. Реализованы три теста:
...
Проверки осуществляются с помощью пакета `testify`.
